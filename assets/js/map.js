document.addEventListener("DOMContentLoaded", function () {
    ...
    function drawLine(e) {
      ...
      if (!startPoint) {
        ...
        line.style.cssText = `
                    ...
                    transform: rotate(${calculateAngle(startPoint, {
                      x,
                      y,
                    })}deg);
  ...
  function initMap() {
    ...
    const map = new google.maps.Map(document.getElementById("map"), {
      center: { lat: 39.76802012578691, lng: 64.45567267188112 }, // Center based on KML data
      zoom: 5, // Adjust the zoom level
    });

    // Load the KML file
    const kmlLayer = new google.maps.KmlLayer({
      url: "URL_TO_YOUR_KML_FILE", // Replace with the URL to your KML file
      map: map,
    });

    // Overlay the existing image on the map
    const imageBounds = {
      north: 39.76802012578691, // Adjust these bounds based on your image
      south: 34.0,
      east: 65.0,
      west: 63.0,
    };
    ...
    // Retain existing popups and ensure they work as before
    // Add your existing popup logic here
  }
